...
def render_world(xyr, xzr, yzr, imgxOutput, imgyOutput):
  ...
  xz = -pi2 * xzr
  yz = -pi2 * yzr
  sxy = np.sin(xy); cxy = np.cos(xy)
  sxz = np.sin(xz); cxz = np.cos(xz)
  ...
  for yi in range(imgyOutput):
    for xi in range(imgxOutput):
      ...
      yio = yi - yo
      zio = 0.0 - zo
      dio = np.sqrt(xio * xio + yio * yio + zio * zio)
      xl = xio / dio
      yl = yio / dio
      zl = zio / dio
      ...
      if val >= 0: # if there is line-sphere intersection
        if val == 0: # 1 intersection point
          ...
          d = min(-dot + np.sqrt(val), -dot - np.sqrt(val))
          xd = xo + xl * d
          yd = yo + yl * d
          zd = zo + zl * d
          x = (xd - xc) / r
